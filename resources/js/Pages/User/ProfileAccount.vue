...
<template>
    ...
    <CropperModalCover v-if="isCoverModalOpen" @showModal="closeCoverModal" />

    <MainNavLayout>
        <div class="w-full min-h-[100vh] pb-20 bg-[#F1F2F5]">
            <div class="w-full bg-white">
                <div class="max-w-[1100px] lg-h-[200px]  pt-[56px] mx-auto pb-1">
                    <!-- cover profile account -->
                   <div class="relative w-full">
                        <img
                            class="rounded-b-xl h-[150px] lg:h-[400px] w-full object-cover"
                            :src="user.cover_image ? `/storage/${user.cover_image}` : 'https://img.goodfon.com/original/1920x1080/0/86/kambodzha-flag-cambodia-flag.jpg'"
                        />
                        <button
                            v-if="$page.props.auth.user.id === user.id"
                            @click="openCoverModal"
                            class="absolute right-4 top-4 bg-gray-200 hover:bg-gray-300 p-2 rounded-full cursor-pointer"
                        >
                            <Camera :size="30" />
                        </button>
                        </div>
